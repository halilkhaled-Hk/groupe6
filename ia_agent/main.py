...
def detect_git_errors() -> list[str]:
    """Détecte la présence de secrets GitHub et de fichiers .env suivis"""
    errors = []

    # Cherche des tokens GitHub dans le code
    result = subprocess.run(["git", "grep", "-i", "ghp_"], capture_output=True, text=True)
    if result.stdout:
        errors.append("Token GitHub détecté :\n" + result.stdout)

    # Vérifie la présence de .env dans l'index
...
def create_fix_branch(base_branch: str) -> str:
    """Crée automatiquement une branche de correction IA"""
    new_branch = f"fix/{base_branch}-{int(time.time())}"
    subprocess.run(["git", "checkout", "-b", new_branch])
    subprocess.run(["git", "add", "."])
    subprocess.run(["git", "commit", "-m", "Fix automatique IA"])
    subprocess.run(["git", "push", "--set-upstream", "origin", new_branch])
    return new_branch


def create_error_branch(base_branch="main") -> str:
    """Crée une branche pour corriger les erreurs de secrets/détection"""
...
def commit_and_push(branch: str):
    """Commit et push des modifications sur la branche spécifiée"""
    subprocess.run(["git", "add", "."])
...
def open_pull_request(branch: str):
    """Crée une pull request via GitHub CLI"""
    subprocess.run([
        "gh", "pr", "create",
        "--base", "main",
        "--head", branch,
        "--title", "Correction automatique",
        "--body", "Cette PR corrige les erreurs détectées."
    ])
...
def main():
    ...
    # Vérification des secrets et .env
    git_errors = detect_git_errors()
    ...
    if errors:
        ...
        if current_branch in ["main", "prod"]:
            ...
            print("Erreurs détectées. Commit bloqué.")
            # Crée une branche de correction pour les autres cas
            new_branch = create_error_branch(current_branch)
            commit_and_push(new_branch)
            open_pull_request(new_branch)
        sys.exit(1)
    else:
        print("Aucun problème détecté. Commit autorisé.")
        sys.exit(0)
